...
> {
  ...
  if (!phone) {
    return {
      ok: false,
      error: "Enter a complete phone number in international format, for example +919876543210.",
    };
  }
  ...
  if (!c) {
    return {
      ok: false,
      error: `Voice is not configured: ${deps.missing().join(", ")} unset.`,
    };
  }

  const now = deps.now();
  const tooSoon = deps.readSends(site.key).some(
    (entry) =>
  ...
  if (tooSoon) {
    return {
      ok: false,
      error: "A test call was just attempted. Wait 30 seconds before placing another.",
    };
  }
  ...
  if (violations.length > 0) {
    return { ok: false, error: "The fixed test-call script did not pass Chapman's speech bounds." };
  }
  ...
  if (!audio.ok) return { ok: false, error: `Sarvam could not render the test: ${audio.error}` };
  ...
  if (!result.ok || !result.ref) {
    return { ok: false, error: result.error ?? "Twilio did not queue the test call." };
  }
