...
import { Block, Figure, Figures, Note, Page, PageHead } from "../components/console";
...
const MAP_FACTS: MapFact[] = [
  { id: "catalogue", y: 74, label: "What you sell", sub: "shape only", src: "feed", crosses: true },
  { id: "policies", y: 136, label: "Policies", sub: "returns · shipping · cod", src: "feed", crosses: true },
  { id: "voice", y: 198, label: "Voice", sub: "the one typed field", src: "typed", crosses: true },
  { id: "notices", y: 260, label: "Service notices", sub: "whole sentences", src: "analysis", crosses: true },
  { id: "constraints", y: 358, label: "Your limits", sub: "margin · discount", src: "typed", crosses: false },
  { id: "commerce", y: 420, label: "Trade", sub: "volume · average", src: "orders", crosses: false },
  { id: "decisions", y: 482, label: "Said & stopped", sub: "replies · refusals", src: "ledger", crosses: false },
  { id: "findings", y: 544, label: "Last analysis", sub: "incidents · ideas", src: "analysis", crosses: false },
];

const MONO = "var(--font-family-code, ui-monospace, monospace)";

function DerivationMap({ state }: { state: Record<string, { present: boolean; stale: boolean }> }) {
  ...
  return (
    ...
    >
      <title>How the cortex is assembled, and what crosses the shopper boundary</title>
      ...
      {[
        ...
        <text
          ...
          style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: "0.12em", textTransform: "uppercase" }}
        >
          {h.t}
        </text>
      ))}
      ...
      {/* The boundary itself. */}
      <line x1={WALL} y1={36} x2={WALL} y2={580} stroke="var(--color-border-emphasized)" strokeWidth={1} strokeDasharray="2 5" />
      <text
      ...
      {MAP_FACTS.filter((f) => !f.crosses).map((f) => (
        <g key={`b-${f.id}`} opacity={edgeOpacity(f.id)}>
          ...
          />
          {/* It stops here. */}
          <rect x={WALL - 16} y={f.y - 9} width={3.5} height={18} fill="var(--color-error)" />
        </g>
      ))}
      ...
      {MAP_FACTS.map((f) => {
        ...
        return (
          ...
          >
            ...
            />
            <text x={336} y={f.y - 2} fill="var(--color-text-primary)" style={{ fontSize: 12, fontWeight: 600 }}>
              {f.label}
            </text>
            <text
              x={336}
      ...
      <rect x={744} y={137} width={168} height={60} rx={8} fill="var(--color-accent)" />
      <text
        x={828}
        y={162}
        textAnchor="middle"
      ...
      <text x={744} y={444} fill="var(--color-error)" style={{ fontSize: 13, fontWeight: 700 }}>
        Nothing arrives.
      </text>
      <text x={744} y={464} fill="var(--color-text-secondary)" style={{ fontSize: 11.5 }}>
        Not withheld by instruction —
      </text>
      <text x={744} y={480} fill="var(--color-text-secondary)" style={{ fontSize: 11.5 }}>
 ...
 */
type GapRow = { who: "you" | "us"; what: string; unlocks: string } & Record<string, unknown>;
...
export default function Cortex() {
  ...
  const mapState: Record<string, { present: boolean; stale: boolean }> = {
    ...
    notices: { present: shopper.serviceNotices.length > 0, stale: stale.notices },
    ...
    <Page>
      ...
      >
        <Figures>
          ...
          <Figure value={c.catalogue.value.categories.length} label="categories" />
          <Figure
      ...
      <Block title="How it is assembled">
          <Card padding={3}>
            <VStack gap={3}>
              ...
              <HStack gap={5} vAlign="center">
                ...
                <HStack gap={2} vAlign="center">
                  <StatusDot variant="warning" label="Past its freshness window" />
          ...
          <Note>
            The dashed line is not a diagram convention — it is a function. shopperView() takes the
            cortex and returns six fields. The four stops on the right are the four it does not
            return, so the assistant cannot leak them the way it cannot leak a file it was never
            opened.
          </Note>
      </Block>

      {/* Shopper-visible */}
      <Block title="The assistant can see this" hint="Everything a shopper-facing reply could possibly be built from.">
            ...
            <MetadataList label={{ position: "start", width: 180 }}>
              ...
              <MetadataListItem label="Voice">
                {shopper.voice ? (
                  ...
                  <Text color="disabled">not set — the assistant sounds generic without it</Text>
                )}
              </MetadataListItem>
                ...
                {shopper.serviceNotices.length > 0 ? (
                  <VStack gap={2}>
                    ...
                    <Text type="supporting">
                      Written here, not by the model. The assistant may repeat one word for word when
                      it is relevant, and cannot reach the finding underneath it — not the failure
                      rate, not the date.
                    </Text>
                    <Src
      ...
      {/* Merchant-only */}
      <Block title="Only you can see this" hint="Your negotiating position. None of it has a route to a shopper-facing reply.">
            ...
            <MetadataList label={{ position: "start", width: 180 }}>
              ...
              <MetadataListItem label="Trade">
                {c.commerce ? (
                  <VStack gap={1}>
                    <Text>
                      ~{c.commerce.value.ordersPerMonth} orders/month · average {shopper.currency}{" "}
                      {c.commerce.value.avgOrderValue} · best sellers:{" "}
                ...
                {c.findings ? (
                  <VStack gap={1}>
                    <Text>
                      ...
                      {c.findings.value.topActions.length} ideas ranked · {c.findings.value.stopped}{" "}
                      stopped
                    </Text>
                    {c.findings.value.recovery ? (
                      <Text type="supporting">
                        ...
                        {c.findings.value.recovery.marginAtStake.toLocaleString("en-IN")} of margin
                        at stake
                      </Text>
                    ) : null}
                  ...
                  <Text color="disabled">
                    never run — open Offers once and the proposer publishes what it finds back to
                    here
                  </Text>
                )}
              </MetadataListItem>
      ...
      {/* Gaps */}
      <Block title="What is still missing" hint="The gaps the agent keeps about itself, and who has to close each one.">
          <Card>
            <Table<GapRow>
              data={gapRows}
              density="balanced"
              columns={[
                {
                  ...
                  width: pixel(84),
                  renderCell: (row) => (
                    <Token size="sm" color={row.who === "you" ? "yellow" : "gray"} label={row.who} />
                  ),
                },
      ...
      <Note>
        This cortex belongs to {c.key} and nothing else reads it. Run two shops and you get two,
        with no path between them — even if the same person owns both.
      </Note>
    </Page>
  );
}
