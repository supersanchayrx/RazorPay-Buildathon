...
import { ensureHighlightStyles, TOKEN_TYPES } from "./highlightStyles.js";
 ...
 */
const registeredHighlightTypes = new Set(TOKEN_TYPES);
 ...
 */
function ensureDynamicHighlightType(tokenType) {
 ...
 */
function createHighlightResolver() {
  const cache = new Map();
  return tokenType => {
 ...
 */
function applyLineRanges(lineDiv, tokens, results, resolve) {
  if (tokens.length === 0) {
    return;
  }
 ...
 */
function applyRangesToContainer(container, tokenLines, globalLineOffset, resolve) {
  const results = [];
 ...
 */
export function applyHighlightRangesChunked(codeEl, tokenLines) {
  ensureHighlightStyles();
  ...
  function applyChunk(wrapper, index) {
 ...
 */
export function applyHighlightRangesBatch(codeEl, tokenLines, startLine) {
  ensureHighlightStyles();
  const resolve = createHighlightResolver();
  const lineDivs = codeEl.querySelectorAll('[data-line]');
  const results = [];
 ...
 */
export function applyHighlightRangesFlat(el, tokenLines) {
  ensureHighlightStyles();
  const resolve = createHighlightResolver();
  const myRanges = [];
