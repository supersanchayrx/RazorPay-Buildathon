...
function matchLinkDefinition(line) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (atBoundary) {
      ...
      if (def != null) {
        ...
        if (!def.hasTitle && index + 1 < lines.length && LINK_TITLE_ONLY_RE.test(lines[index + 1])) {
          keep[index + 1] = false;
          index++;
        }
...
function linkDefsSignature(defs) {
  if (defs.size === 0) {
    return '';
  }
  return [...defs].sort((a, b) => a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0).map(([label, dest]) => `${label}\u0000${dest}`).join('\u0001');
}
 ...
 */
function matchReferenceLink(text, start, linkDefs, opts) {
      ...
      if (href != null && isSafeUrl(href)) {
        return {
          node: {
            type: 'link',
            href,
            children: parseInlineImpl(linkText, opts)
          },
          end: labelClose + 1
        };
      }
...
function matchReferenceImage(text, start, linkDefs) {
 ...
 */
function matchFullwidthCitation(text, i, opts) {
 ...
 */
function matchBracketCitation(text, i, opts) {
  if (!opts.sourceIds || text[i] !== '[') {
    return null;
  }
  const closeIndex = text.indexOf(']', i + 1);
...
export function parseInline(text, arg) {
  ...
  while (i < text.length) {
    ...
    if (text[i] === '`') {
      ...
      if (closeIndex !== -1) {
        nodes.push({
          type: 'code',
          content: text.slice(openIndex, closeIndex)
        });
...
const EMAIL_LOCAL_CHARS = new Set('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._%+-@');
/**
 ...
 */
function peelTrailingPunctAndParens(url) {
  let s = url;
  while (s.length > 0) {
...
const URL_CONTINUATION_CHARS = new Set('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789/=');
 ...
 */
function isUrlBoundaryChar(ch) {
  if (ch == null) {
    return true;
  }
  return !URL_CONTINUATION_CHARS.has(ch);
}
function scanAutolinksInText(text) {
  const matches = [];
 ...
 */
function splitTextOnAutolinks(content) {
  ...
  for (const m of matches) {
    if (m.start > cursor) {
      out.push({
        type: 'text',
        content: content.slice(cursor, m.start)
      });
    }
    out.push({
      type: 'link',
 ...
 */
function transformAutolinks(nodes) {
  const out = [];
  for (const node of nodes) {
    if (node.type === 'text') {
      ...
      out.push({
        ...node,
        children: transformAutolinks(node.children)
      });
    } else {
      out.push(node);
    }
 ...
 */
function nested(opts) {
  return opts.sourceRanges ? {
    ...opts,
    sourceRanges: false
  } : opts;
}
...
function parseTable(lines, lineIndex, opts) {
  const headers = splitTableRow(lines[lineIndex]).map(cell => ({
    children: parseInlineEntry(cell, opts)
  }));
  const alignments = splitTableRow(lines[lineIndex + 1]).map(cell => {
  ...
  while (rowIndex < lines.length && lines[rowIndex].includes('|') && lines[rowIndex].trim() !== '') {
    rows.push(splitTableRow(lines[rowIndex]).map(cell => ({
      children: parseInlineEntry(cell, opts)
    })));
    rowIndex++;
  }
...
function parseList(lines, startIndex, ordered, opts) {
  const items = [];
  ...
  const itemPattern = ordered ? new RegExp(`^ {${baseIndent}}\\d+${escDelim} `) : new RegExp(`^ {${baseIndent}}[-*+] `);
  const start = orderedStart ? parseInt(orderedStart[1], 10) : undefined;
  let loose = false;
  let index = startIndex;
  while (index < lines.length && itemPattern.test(lines[index])) {
    const content = ordered ? lines[index].replace(new RegExp(`^ *\\d+${escDelim} `), '') : lines[index].replace(/^ *[-*+] /, '');
  ...
  return {
    node: {
      ...
      ordered,
      start,
      delimiter: ordered ? delim : undefined,
...
export function parseMarkdown(input, arg) {
  return parseMarkdownImpl(input, resolveOptions(arg));
}
function parseMarkdownImpl(input, baseOpts) {
  ...
  if (defs.size === 0) {
    ...
    linkDefs = new Map([...defs, ...inherited]);
  }
  const opts = linkDefs != null ? {
    ...baseOpts,
    linkDefs
  } : baseOpts;
  ...
  while (index < lines.length) {
    ...
    if (imageMatch && line.trim() === imageMatch[0] && isSafeUrl(imageMatch[2])) {
      pushBlock({
        type: 'image',
        alt: imageMatch[1],
        src: imageMatch[2]
  ...
  if (blockStartLines != null) {
    stampSourceRanges(blocks, blockStartLines, blockEndLines ?? [], lines, lineMap, input, opts);
  }
  return blocks;
}
 ...
 */
function stampSourceRanges(blocks, blockStartLines, blockEndLines, lines, lineMap, input, opts) {
  ...
  const lineStart = line => base + inputLineStarts[lineMap != null ? lineMap[line] : line];
  for (let i = 0; i < blocks.length; i++) {
    const startLine = blockStartLines[i];
    let endLine = blockEndLines[i];
    if (endLine == null) {
      const nextStart = i + 1 < blocks.length ? blockStartLines[i + 1] : lines.length;
      endLine = nextStart - 1;
 ...
 */
function findSettledBoundary(lines) {
  let inFence = false;
  let fenceMarker = '';
  let lastBoundary = -1;
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    if (!inFence && line.trim() === '' && lineIndex > 0 && lineIndex < lines.length - 1) {
 ...
 */
export function trimStreamingArtifacts(input) {
 ...
 */
function trimUnsettledStructural(text) {
 ...
 */
function mergeSettledBlocks(prev, delta) {
  if (prev.length === 0 || delta.length === 0) {
    return [...prev, ...delta];
  }
  const prevLast = prev[prev.length - 1];
  const deltaFirst = delta[0];
  if (prevLast.type === 'list' && deltaFirst.type === 'list' && prevLast.ordered === deltaFirst.ordered && prevLast.delimiter === deltaFirst.delimiter) {
    const merged = {
      type: 'list',
      ...
      ...(prevLast.range != null && deltaFirst.range != null ? {
        range: {
          start: prevLast.range.start,
          end: deltaFirst.range.end
        }
      } : null)
    };
...
export function parseMarkdownIncremental(input, state, arg) {
  ...
  // be reused unchanged.
  if (state.autolink !== opts.autolink || Boolean(state.sourceRanges) !== Boolean(opts.sourceRanges)) {
    state.prevInput = '';
    state.settledText = '';
    state.settledBlocks = [];
    state.settledUpTo = 0;
  ...
  const parseOpts = linkDefs.size > 0 ? {
    ...opts,
    linkDefs
  } : opts;
  const lines = input.split('\n');
  ...
  const unsettledText = openFence ? unsettledRaw : trimUnsettledStructural(unsettledRaw);
  let settledBlocks;
  if (settledText === state.settledText) {
    ...
    const deltaBlocks = parseMarkdownImpl(delta, atOffset(parseOpts, state.settledText.length));
  ...
  const unsettledStart = unsettledText && opts.sourceRanges ? input.indexOf(unsettledText, settledText.length) : -1;
  const unsettledBlocks = unsettledText ? parseMarkdownImpl(unsettledText, unsettledStart >= 0 ? atOffset(parseOpts, unsettledStart) : nested(parseOpts)) : [];
