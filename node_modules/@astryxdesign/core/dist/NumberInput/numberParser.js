...
function getLocaleNumberSymbols(locale) {
  const key = locale ?? '';
  ...
  if (symbols === undefined) {
    ...
    const groupSizes = format.formatToParts(11111111111).filter(part => part.type === 'integer').map(part => part.value.length);
 ...
 */
function readUnder(core, group, decimal, primaryGroupSize, secondaryGroupSize, minGroups) {
  let intText = core;
  let fracText = null;
 ...
 */
function isGroupOnlySeparator(separator) {
  return /^[\s'\u2019]$/.test(separator);
}
function toPlainNumber(core, locale) {
...
export function parseLocaleNumber(text, locale) {
  ...
  const scaled = exponent === 0 ? magnitude :
 ...
 */
export function formatEditableNumber(value, locale) {
  const {
    decimal
  } = getLocaleNumberSymbols(locale);
