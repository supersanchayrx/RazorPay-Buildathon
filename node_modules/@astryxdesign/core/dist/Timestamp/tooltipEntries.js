 ...
 */
const warnedTimezoneIDs = new Set();
 ...
 */
function resolveTimezoneID(timezoneID) {
  ...
  try {
    ...
    if (!warnedTimezoneIDs.has(timezoneID)) {
      ...
      devWarn('Timestamp', `unknown time zone ${JSON.stringify(timezoneID)} in tooltipEntries. Falling back to the viewer's time zone.`);
    }
    return undefined;
  }
  return timezoneID;
 ...
 */
function shouldShowZoneName(format, hasMultipleZones, isNamedZone) {
  if (format === 'full') {
    return true;
  }
 ...
 */
export function formatTooltipLines(date, entries, locale) {
