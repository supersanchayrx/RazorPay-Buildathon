...
export function Tokenizer({
   ...
   */
  const createEntries = useCallback((query, results) => {
    const trimmed = query.trim();
    if (!hasCreate || trimmed === '') {
      return [];
    }
    const alreadyExists = selectedIds.has(trimmed) || results.some(item => item.label.toLowerCase() === trimmed.toLowerCase());
    if (alreadyExists) {
      return [];
    }
    return [{
  ...
  const handleAdd = useCallback(item => {
    ...
    if (hasCreate && typeof item.id === 'string' && item.id.startsWith(CREATABLE_ID_PREFIX)) {
      ...
      announce(t('@astryx.tokenizer.tokenAdded', {
        label: createdValue
      }));
      return;
    }
  ...
  const handleRemove = useCallback(item => {
    ...
    announce(t('@astryx.tokenizer.tokenRemoved', {
      label: item.label
    }));
    inputRef.current?.focus();
  ...
  const wrapperContent = /*#__PURE__*/_jsxs("div", {
    ...
    }), stylex.props(inputWrapperStyles.base, styles.wrapper, value.length > 0 && styles.wrapperWithTokens, isTruncated ? truncatedSizeStyles[size] : sizeStyle, isTruncated && styles.truncatedWrapper, isDisabled && inputWrapperStyles.disabled, status && inputStatusBorderStyles[status.type], status && !isDisabled && inputStatusHoverShadowStyles[status.type], status && inputStatusFocusWithinStyles[status.type])),
    ...
    tokenizerContent = /*#__PURE__*/_jsxs(_Fragment, {
      children: [/*#__PURE__*/_jsx("div", {
        ...
        ...mergeProps(themeProps('tokenizer', {
          ...
          status: status?.type,
          disabled: isDisabled ? 'disabled' : null
        }), stylex.props(inputWrapperStyles.base, styles.wrapper, value.length > 0 && styles.wrapperWithTokens, placeholderSizeStyle, isTruncated && styles.truncatedWrapper, isDisabled && inputWrapperStyles.disabled, status && inputStatusBorderStyles[status.type], status && !isDisabled && inputStatusHoverShadowStyles[status.type], status && inputStatusFocusWithinStyles[status.type])),
